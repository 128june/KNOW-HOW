...
| 데이터 리니지 | `#data-lineage` | 실제 입수·가공 작업, 마트·결과, KB 연결과 기록된 근거 사용을 따라가기 |
...
**2026-09-22 허브 확인 기준:** 공개 UI `0cd64e5`와 API `5f4cb58`의 배포·공개 동작이 확인되었습니다. Mac 호스트의 같은 API 버전에서 GS타워 문서에 대한 실제 AI 문의가 193자의 초안을 생성했고, 해당 실행의 독립 의미 검증을 통과했습니다. 이 결과는 해당 시나리오의 검증이며 모든 질문의 품질이나 실제 임직원의 업무 효과를 입증하지 않습니다. 이 브랜치의 리니지 기능은 별도 로컬 검증 대상이며 허브 통합·배포 전에는 공개 완료로 보지 않습니다.

생성은 서버의 `codex_cli` 호스트 경로, 임베딩은 승인된 OpenAI 연결(key2)을 사용하며 key1은 사용하지 않습니다. 최근 GS타워 조회는 기존 임베딩 캐시를 재사용해 새 임베딩 호출이 없었고, 이전 검증에서는 실제 임베딩 실행을 확인했습니다. 이는 CLI 자체가 임베딩을 제공한다는 의미가 아닙니다. 이번 리니지 구현·검증은 새 AI 생성·임베딩 호출 없이 진행하며 공개 서비스의 모델·설정·예산을 변경하지 않습니다. 운영 절차는 API 저장소의 `docs/knowhow-codex-cli.md`를 따릅니다.

리니지는 현재 방문자 공간의 실제 메타데이터를 조회합니다. 카드를 선택하면 원본·결과 ID, 행 수, 실행 시각, 실패·미완료 상태, 규칙과 접근 가능한 정책 버전을 확인하고 원본·결과 화면 또는 KB 문서로 이동할 수 있습니다. KB 연결과 저장된 대화의 근거 사용은 별도 노드입니다. 일반 데이터 조회·다운로드 이력, 사용 시각·당시 문서 버전처럼 저장하지 않는 정보는 추정하지 않습니다. 자료·부서 선택으로 범위를 좁힐 수 있고, 서버 조회 한도에 도달하면 잘림을 표시합니다. 토큰은 같은 탭의 메모리에만 유지하며 새로고침 후에는 자료 가져오기에서 다시 시작합니다.
...
| `src/data-platform.js`, `src/data-review-ui.js`, `src/data-lineage-ui.js` | 실제 입수와 처리·정책 검토 |
