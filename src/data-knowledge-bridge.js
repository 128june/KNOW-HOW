...
(function(root){'use strict';
 ...
 function createController({apiBase,dataApiBase}={}){
  ...
  function actions(){
   ...
   target.innerHTML=`${!connected?`<button class="primary" data-knowledge-action="connect" ${disabled}>${state.busy?'연결 중…':'지식을 남길 부서 선택하기'}</button>`:`<p class="muted">아래 가상 조직·부서를 선택한 뒤 초안을 만드세요. 저장 범위는 선택한 부서이며, 담당자 확인 전에는 질의 근거로 사용되지 않습니다.</p><button class="primary" data-knowledge-action="draft" ${disabled}>${state.busy?'처리 중…':'선택한 부서에 데이터 가이드 초안 만들기'}</button>`}${state.draft?`<p class="notice">초안으로 저장한 문서 v${esc(state.draft.version)}입니다. 현재 확인 상태는 아래 문서에서 확인하세요. 전사 공유는 별도 승인 절차입니다.</p><details><summary>문서 추적 정보</summary><p>조회 ID: <code>${esc(state.draft.identifier||state.draft.id)}</code></p></details><div class="actions"><button data-knowledge-action="review" ${disabled}>저장한 초안 검토하기</button><button data-knowledge-action="query" ${disabled}>이 데이터 가이드에 질문하기</button><button data-knowledge-action="manifest" ${disabled}>서버가 검증한 데이터 출처 보기</button></div>${state.view==='chat'?'<p class="muted">저장한 문서를 질문 대상으로 선택했습니다. 담당자 확인을 마친 현재 버전을 조회하며, 기본 조회는 AI 생성 없이 근거를 표시합니다.</p>':''}`:''}<p role="status">${esc(state.notice)}</p>${state.error?`<p role="alert" class="scope-reason">${esc(state.error)}</p>`:''}${state.manifest?`<details open><summary>서버 출처 확인 · ${esc(state.manifest.status||'확인됨')}</summary><pre class="source">${esc(JSON.stringify(state.manifest.manifest||{},null,2))}</pre></details>`:''}`;
  ...
  function render(){
   ...
   host.innerHTML=`<section class="card data-knowledge-bridge"><h2>데이터 구조·업무 가이드</h2><p>데이터의 열 구조, 처리 규칙과 원본부터의 연결을 조직 지식으로 남깁니다.</p>${!state.expanded?'<button class="primary" data-knowledge-action="reveal">이 데이터의 의미를 지식으로 남기기</button>':`<details><summary>저장되는 내용과 사용 범위</summary><p class="notice">공개 체험의 가상 데이터를 선택한 가상 조직·KB 세션의 가이드 초안으로 연결합니다. 데이터 공개 범위와 KB 세션은 별개이며, 이 연결은 서버가 확인합니다. 실제 조직의 로그인 자료로 연결되지 않습니다.</p><p class="muted">샘플 값과 원본 개인행은 보내지 않습니다. 서버가 허용한 스키마·가공 규칙·원본 식별자·해시·생성 시각·계보만 출처로 보존합니다. 출처가 만료되거나 접근할 수 없으면 확인·공유·질의 근거에서 제외됩니다.</p></details><div data-knowledge-region="actions"></div><div data-knowledge-region="organization"></div>`}</section>`;
