...
(function(root){'use strict';
 ...
 const readableContent=v=>esc(v).replace(/\. /g,'.<br>').replace(/; /g,';<br>');
 const seeds=[
  {
    ...
    "scenario_version": 2,
    "content": "가상 정책·DB; 실제 회계기준 아님. 적용 2026-09-01~12-31, 다른 기간·계약 소급 금지. 운영매출=payments.paid_at의 한국시간 월; settlements.settled_at은 입금대사만(9/30결제·10/2정산→9월매출). 취소·테스트 제외. 부분환불은 refunds.completed_at 월에 환불액 차감. 재무팀=기준·계약예외 확정, 데이터팀=집계식 관리. 근거 REV-2026-09 §2·4, 가상확인 2026-09-21(실제승인 아님).",
    "correction": "가상 정책·DB; 실제 회계기준 아님. 적용 2026-09-01~12-31, 다른 기간·계약 소급 금지. 운영매출=payments.paid_at의 한국시간 월; settlements.settled_at은 입금대사만(9/30결제·10/2정산→9월매출). 취소·테스트 제외. 전액·부분환불은 refunds.completed_at 월에 실제 환불액 차감, 미완료 제외. 9월 결제10만원을10월 전액환불→9월 유지·10월10만원 차감. 재무팀=기준·계약예외 확정, 데이터팀=집계식 관리. 근거 REV-2026-09 §2·4, 가상확인 2026-09-21(실제승인 아님).",
    "owner": "재무팀",
    "category": "매출 집계 기준",
  ...
  },
  {
    ...
    "scenario_version": 2,
    "content": "가상 정책·DB. 적용 2026-09-01~12-31, 다른 기간·계약 적용 금지. 월 결제완료·기준일 유효 identity_map.canonical_customer_key를 중복제거. account_id=로그인계정, legal_entity_id=계약법인; 집계키 아님. 같은 고객키의 여러 계정=1명, 같은 법인도 다른 고객키=별도. 미매핑·상충은 미확정 별도표시, 임의병합 금지. 테스트·취소 제외. 데이터팀=매핑, 고객운영팀=동일고객 확인. 근거 CUST-2026-09 §1·3, 가상확인 2026-09-21(실제승인 아님).",
    "correction": "가상 정책·DB. 적용 2026-09-01~12-31, 다른 기간·계약 적용 금지. 월 결제완료·기준일 유효 identity_map.canonical_customer_key를 중복제거. account_id=로그인계정, legal_entity_id=계약법인; 집계키 아님. 같은 고객키의 여러 계정=1명, 같은 법인도 다른 고객키=별도. 과거 재계산=당시 연결표, 최신키 대체 금지. 법인명·이메일만으로 병합 금지. 미매핑·상충·근거없음=미확정 별도표시. 테스트·취소 제외. 데이터팀=매핑, 고객운영팀=동일고객 확인. 근거 CUST-2026-09 §1·3, 가상확인 2026-09-21(실제승인 아님).",
    "owner": "데이터팀",
  ...
  },
  {
    ...
    "scenario_version": 2,
    "content": "가상 정책·DB; 실제 약관 아님. 적용 2026-09-01~12-31, 소급·다른 계약 적용 금지. refunds.requested_at 당시 계약·확인사유: 회사귀책·중복결제=회사부담; 고객변심=고객부담 조항 있을 때만 고객부담. 조항·귀책 미확정=확인필요. PG수수료 면제 확인→0원. 고객운영팀=사유·접수시각 확인, 재무팀=계약예외·면제증빙 확정. refund_reason≠귀책·면제 증빙. 근거 REF-2026-09 §2·5, 가상확인 2026-09-21(실제승인 아님).",
    "correction": "가상 정책·DB; 실제 약관 아님. 적용 2026-09-01~12-31, 소급·다른 계약 적용 금지. refunds.requested_at 당시 계약·확인사유: 회사귀책·중복결제=회사부담; 고객변심=고객부담 조항 있을 때만 고객부담. 조항·귀책 미확정=확인필요. 신청≠확정. 재무팀 증빙확정 면제액만 차감, 잔액은 기존 부담기준(전액면제0원; 고객부담3000원·면제1000원→2000원). 고객운영팀=사유·접수시각 확인, 재무팀=계약예외·면제증빙 확정. refund_reason≠귀책·면제 증빙. 근거 REF-2026-09 §2·5, 가상확인 2026-09-21(실제승인 아님).",
    "owner": "고객운영팀",
    "category": "환불 수수료 예외",
 ...
 function createController(){
  ...
  function source(d){const original=seeds.find(s=>s.id===d.id);return `<details><summary>원문·확인 근거·정정 이력</summary><p>${esc(original.filename)} · 모든 규칙·수치·자료는 가상 예시입니다.</p><h3>처음 받은 자료</h3><pre class="source">${esc(original.raw)}</pre><h3>담당 부서가 확인한 최초 기준</h3><pre class="source">${readableContent(original.content)}</pre><h3>정정 이력</h3><p>최신 본문은 v${d.versions[0].version}입니다. 최초 원문은 그대로 유지합니다.</p>${d.versions.map(v=>`<p><strong>v${v.version}</strong> · ${esc(v.reason)}</p><pre class="source">${readableContent(v.content)}</pre>`).join('')}</details>`}
