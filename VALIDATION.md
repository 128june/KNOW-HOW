...
- 실제 브라우저 1440/390/320px에서 홈 네 항목의 라벨·제목·본문 겹침이 모두 0이었다.
- 홈/시나리오2/범용 사례/데이터 시작 화면의 12조합에서 가로 넘침 0, 헤더 48/56px 및 본문 상단 좌표는 수정 전과 같았다.
- source 복사본의 임시 서버에서 외부 연결을 차단해 검사했다. 모델·입수·KB 요청은 없었으며 공개 배포 완료 증거는 아니다.
- 허브 근거: `01_현재_아이디에이션/evidence/mobile-home-fix/REPORT.md`, 원본 좌표와 화면. 전체 공개 전환은 API0b1b5c3 배포 검증 후 별도로 수행한다.

## 현재 후속 검증 · AI 요청 중지와 업무 적용일

- **AI 중지 `874d267`**: Pages workflow가 `KNOWHOW_AI_PAUSED`를 전달하고, 설정이 없으면 중지를 유지한다. 잘못된 값은 기존 dist를 삭제하기 전에 빌드를 실패시킨다. 개인정보 AI 제안은 버튼과 요청 핸들러 모두 차단하며 기존 결과를 지우지 않는다. 정책 원문 조회·직접 처리 규칙은 계속 사용한다. 브라우저 제어이며 서버 권한·예산 제한을 대체하지 않는다.
- **업무일과 입력 보존**: 새 방문자 입수 문맥은 한국시간 오늘을 기본 조회일로 사용한다. 같은 문맥에서 고른 과거 날짜는 유지하고 다른 자료는 새 업무일로 시작한다. 날짜만 바꿀 때 선택 문서·질문을 보존하고 이전 날짜의 대화·늦은 답변을 분리한다. KB 모음에서 날짜를 바꾸면 이전 조회 화면의 답변도 지운다. 기존 고정 시나리오 날짜는 별도로 표시한다.
- **API `e72e30f`**: 서버 시간대와 무관하게 생략한 기준일·현재 공유 유효성·개인정보 정책 적용에 한국 업무일을 쓴다. 명시한 달력 날짜, 원본 수집·감사 시각, TTL은 바꾸지 않는다. 과거 날짜가 과거 문서 버전 복원을 의미하지 않는다.
- **최종 실제 로컬 통합**: 새로운 임시 DB와 UTC 프로세스 API에서 합성 17열·2행 CSV 입수 → 정책 원문 조회(generate false) → 장비부서 KB v1 → 사람의 미확인 사항 기록 v2/별도 검토 완료 → 과거·현재 적용일 조회 → 전사 공유 승인 → 앱부서에서 같은 v2 조회를 확인했다. 문서 `ee4b214df595061fd629df3c41e2380d`, API 응답 46건 모두 2xx(200 42건/201 2건/202 2건), 모델 호출·JS 오류·외부 요청 0. 320px 가로 넘침 0이며 PC·모바일 화면을 직접 확인했다. API 응답 대체는 없고 UI 설정의 연결 주소만 로컬로 지정했다.
- **관련 검사**: AI 설정 빌드 3건, 데이터 입수 상태 11건, 개인정보 검토 UI 8개 계약, 날짜 관련 UI 12개 스크립트 통과. 이후 답변 무효화 수정은 영향 있는 9개 스크립트를 다시 확인했다(중복 합산하지 않음). API 관련 64건(새 자정 경계 4건 포함)이 UTC에서 통과했고 같은 경계 4건은 다른 서버 시간대에서도 통과했다. 허브가 마지막 소스의 날짜 UI·API 경계를 재확인했다. actionlint/Bash/diff 검사와 AI 중지 변경 독립 검토도 통과했다.
- **증거 구분**: 허브 `01_현재_아이디에이션/evidence/release-readiness/`에 응답·소스 해시·화면·재현 코드를 보존했다. 처음의 5열 입력은 기존 17열 계약에 맞지 않은 검사 자료 오류였다. 후속 회사 조회 검사는 성공 응답 기록을 기다리지 않아 실패했으며, 테스트 동기화만 수정해 최종 통과했다. 제품/API 응답을 우회하지 않았다.
- **공개 상태와 남은 범위**: 로컬 커밋만 완료하고 push·공개 배포는 하지 않았다. 현재 공개 UI984f9c7/API420a244는 최신 방문자 입수·정책·협업 계약과 다르므로 최신 UI만 먼저 배포할 수 없다. Mac mini CLI 파일은 발견했지만 실제 버전·로그인·모델 호출과 API 연결은 미검증이다. 실제 AI 5PASS/3FAIL과 전체 목표의 미완료 상태를 유지한다.

## 직전 검증 · 나중에 확인한 연결 근거를 같은 KB에 반영

- 기존 KB의 담당자 정정에 연결표 대조 → 정정 준비 → 명시 저장을 추가했다. 조회·준비·취소는 문서/검토/공유 상태를 변경하지 않는다. 현재 ID와 연결표 ID·테이블/필드를 먼저 보여주고 원본행과 연결 기준 원문은 펼쳐 확인한다.
- 정정 준비는 기존 본문/작성 중 입력/변경 이유/댓글 선택을 보존하며 새 연결 근거를 편집 가능한 본문에 추가한다. 상충하는 기존 문구는 사람이 대조해 정리하고 이유를 적는다. 연결표 ID와 수정 ID가 다르면 API 전에 차단하며, 적용 취소 후 직접 수정할 수 있다. 기존 버전 충돌 방어도 유지한다.
- 기존 source의 원문 메타 전체를 유지하며 새 버전의 부서 ID/status/reference만 추가한다. 과거 source는 변경하지 않는다. 정정 전후에도 버전별 부서ID/연결근거를 표시한다. 방문자 입수 문서는 이 합성 부서표를 적용하지 않는다.
- **실제 로컬 UI/API58bd1d6**: GS타워/GS차지비/공공02의 기존 미연결 문서를 명시적으로 격리 API에 준비 → UI 검토v2/전사승인 → 댓글 → 실제 workflow/inquiry 근거 비교/준비/취소/재준비(저장0) → 같은 문서v3정정 → 사람 검토v4/별도완료 → 전사재승인 → 앱부서의 같은v4 원문조회/새로고침. 새 ID는 DEMO-DEVICE-80fc20c9c26c64aa, device_development_chargers.device_charger_id와 원본행20184를 함께 보존했다.
- 정정 및 검토만 마친 상태에서는 타부서 전사목록에 제외되고, 재승인 후 v4만 조회됐다. v2 원문/source 불변, 현재source1,879/4,000B, 원문dataset/run/SHA/충전소/행 불변, 댓글v4반영. 지식+연결 HTTP64응답 모두200, JS0/모델0/320px넘침0. 최초 미연결 자료는 테스트 준비이며 실제 과거 회사 KB를 사용했다고 주장하지 않는다.
- 관련12회귀 PASS: company-mapping-revision, company-revision-version, company-identity, company-human-review, company-live, company-mapping, company-navigation, company-sharing, company-sharing-transport, company-provenance, demo-isolation, organization-kb. 독립 UI 검토의2개 회귀는 이 부분집합이며 별도 실제DOM/모의API에서도 입력·메타 보존/취소를 확인했다. 최종 표시 정리는 허브가 PC/320px 실제API 브라우저로 다시 검증했다.
- 독립 서버 계약은 임시 DB의 dispatch25회로 과거source보존, 정정 후 전사 stale, 재승인 전 구버전 재사용 없음, 재승인 후 최신본문/매핑source가 모의 생성기로 전달됨을 확인했다. 실제 모델 호출은0이며 source가 client 메타인 기존 계약은 바뀌지 않는다. 내부 연결 사실을 서버가 독립 검증하는 기능은 아니다.
- 허브 `01_현재_아이디에이션/evidence/mapping-revision/`에 실제 응답/화면/재현, 독립API/UI검토를 보존했다. 현재 공개 보존520,460행·합성부서표 범위이며 새 다운로드/실제내부 대응/실AI 정확성/공개배포와 구분한다. 미push·미배포, 기존 실제AI 5PASS/3FAIL 유지.

## 직전 검증 · 동시 정정 후 최신 조건 보존
