...
(async()=>{
 ...
 try{
  ...
  await mount('policies');assert.match(await page.locator('.data-policy-application').innerText(),/정책 1개 적용/);assert.match(await page.locator('.data-application-priority').innerText(),/phone은 연락처/);
  const source=page.locator('.data-application-source'),sourceSummary=source.locator('summary');assert.equal(await source.evaluate(element=>element.open),false);assert.equal(await source.locator('dd').isVisible(),false);await sourceSummary.focus();await sourceSummary.press('Enter');assert.equal(await source.locator('dd').textContent(),raw.source_sha256);await sourceSummary.press('Enter');assert.equal(await source.evaluate(element=>element.open),false);
  const kept=page.locator('.data-application-unchanged');assert.equal(await kept.evaluate(element=>element.open),false);assert.match(await kept.locator('summary').innerText(),/1개 열/);await kept.locator('summary').focus();await kept.locator('summary').press('Enter');assert.equal(await kept.locator('[data-application-column="memo"]').isVisible(),true);await kept.locator('summary').press('Enter');
  await verifyPolicyCard();await measure('policy-result');
  assert.equal(errors.length,0,errors.join('\n'));assert.ok(fixture.calls.every(c=>!c.body.generate));
  const report={passed:true,scope:'API-response-substituted UI transport/render fixture; not actual API/Worker validation',apiResponsesSubstituted:true,aiCalls:0,externalMutations:0,cases:['draft preserves exact original','review/request/approve explicit steps','department exclusion and company opt-in','whole original visible by default before and after application','technical ID/SHA hidden by default, Enter opens and closes exact values','rendered original SHA matches fixture source metadata','per-column manual selection reason','409 preserves entered decisions','priority result reason remains visible and source SHA opens exactly','unchanged decisions stay counted and keyboard-accessible','1440/390/320 full-shell layout'],policySHA256:fixture.docs[0].sha256,measurements,errors,requestCount:fixture.calls.length};
