...
(async()=>{
 ...
 let capture;context.fetch=async(url,options)=>{capture={url,options};return {ok:true,json:async()=>({stations:[]})}};
 await exec("sampleDemo.request('search',{q:'GS타워'})");assert.equal(capture.url,'https://api.example/knowhow/demo/workflow/search');assert.equal(capture.options.credentials,'omit');assert.equal(capture.options.headers.Authorization,undefined);
 exec("sampleDemo.addComment(sampleDemo.doc(),'Please correct the procedure')");assert.equal(exec('sampleDemo.doc().versions[0].version'),1);assert.equal(exec('sampleDemo.doc().comments[0].resolvedVersion'),null,'opinion must not become approved');
 exec("sampleDemo.revise(sampleDemo.doc(),{content:'Corrected unique marker',reason:'Example correction',basis:'Example source',validFrom:'2026-09-01',validTo:'2026-12-31',resolve:[sampleDemo.doc().comments[0].id]})");assert.equal(exec('sampleDemo.doc().versions[0].version'),2);assert.equal(exec('sampleDemo.doc().comments[0].resolvedVersion'),2);assert.equal(exec('sampleDemo.doc().versions[0].identifiers.length'),0);
 exec("sampleDemo.getState().question='A new follow-up';sampleDemo.reuseAnswer()");assert.equal(exec('sampleDemo.getState().answer.content'),'Corrected unique marker');assert.equal(exec('sampleDemo.getState().answer.question'),'A new follow-up');
 exec("sampleDemo.getState().asOf='2027-01-01';sampleDemo.reuseAnswer()");assert.equal(exec('sampleDemo.getState().answer.applicable'),false);
 exec('sampleDemo.activate()');assert.equal(exec('sampleDemo.doc().versions[0].content'),'Corrected unique marker','prepared seeds must not overwrite corrections');assert.equal(storage.get('organization-records'),'untouched');
 exec("sampleDemo.getState().asOf='2026-09-21';sampleDemo.getAI().question='Explain the sample';sampleDemo.getAI().mode='v2'");
 let aiCalls=0;context.fetch=async(url,options)=>{aiCalls++;capture={url,options};return {ok:true,json:async()=>({ai_generated:true,answer:'Generated <sample>',sample_revision:'v2',evidence:[]})}};
 assert.equal(aiCalls,0);await exec('sampleDemo.requestAI()');assert.equal(aiCalls,1);assert.equal(JSON.parse(capture.options.body).sample_revision,'v2');assert.equal(JSON.parse(capture.options.body).browser_note,undefined);assert.equal(capture.options.headers.Authorization,undefined);
 assert.ok(exec('sampleDemo.aiResultMarkup(sampleDemo.getAI().result)').includes('실제 AI 생성'));assert.ok(exec('sampleDemo.aiResultMarkup(sampleDemo.getAI().result)').includes('&lt;sample&gt;'));
